...
| `/servicii` | `src/app/servicii/page.tsx` | `views/services.tsx` → `ServicesView` |
| `/colaborare` | `src/app/colaborare/page.tsx` | `views/collaboration.tsx` → `CollaborationView` |
...
| File | Role |
|------|------|
| `layout.tsx` | Root layout — provider tree, font, `metadata` + `viewport`, JSON-LD, `<PageTransition />` |
| `template.tsx` | Remounts per navigation — pass-through (no enter animation) |
...
> `next/link` directly. Route changes use a light `<PageTransition />` cover veil
> on click — see [[components/common]].
